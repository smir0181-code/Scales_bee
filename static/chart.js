...
async function renderChart() {
    ...
    if (data.length > 0) {
        ...
        if (ageMs <= MAX_AGE_MS) {
            ...
            // Отображаем батарею
            const battery = lastEntry.battery_percent !== undefined && lastEntry.battery_percent !== null
                ? lastEntry.battery_percent
                : '—';
            document.getElementById('currentBattery').innerHTML = battery;
        } else {
            color = 'rgba(148, 146, 150, 0.3)';
            document.getElementById('currentWeight').textContent = '— (нет актуальных данных, датчик вероятно отключён)';
            document.getElementById('currentTime').textContent = '—';
            document.getElementById('currentBattery').innerHTML = '—';
        }
    } else {
...
// Запуск
renderChart();
setInterval(renderChart, 60000);  // Обновление каждые 60 секунд
